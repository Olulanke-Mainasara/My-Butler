...
export function EventForm({ initialData }: { initialData?: Event }) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: initialData
      ? {
          admission_price: initialData.admission_price ?? 0,
          capacity: initialData.capacity ?? 100,
          description: initialData.description ?? "",
          duration_hours: initialData.duration_hours ?? 8,
          end_date: toDatetimeLocal(initialData.end_date),
          exhibitor_count: initialData.exhibitor_count ?? 0,
          featured_speakers_description:
            initialData.featured_speakers_description ?? "",
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardHeader className="px-0 pb-4 pt-0 text-center lg:text-left">
            <CardTitle className="text-4xl">
              {isEditMode ? "Edit Event" : "Post new Event"}
            </CardTitle>
