-- PENDING: written and retried repeatedly this session against a
-- persistent Supabase MCP approval gate that didn't clear. Not yet
-- confirmed applied to the live project. If you have Supabase dashboard/CLI
-- access, this can be run directly in the SQL editor to unblock hybrid
-- brand onboarding without waiting on the MCP tool - once applied, rename
-- this file to match the version Supabase assigns (see `supabase migration
-- list` or the dashboard's Migrations page) so the repo matches reality,
-- the way the six migrations before it already do.
